...
Program Description:
    ...
    in a holding cell, you realize that youre in the clutches of Professor
    ...
    the time?), so you have a chance to examine your cell. Looking around, you
    see no signs of surveillance (ha, they must underestimate you already) and
    the only thing keeping you contained is an electronic door lock. Should be
    easy enough.
    ...
    and with some quiet observation of the guards you find out the results of
...
def answer(digest):
    ...
    # generator for hash dictionary
    def gen_hash_dict(dim):
        for x in range(dim):
            for y in range(dim):
                yield str(y) + '.' + str(hfunc(x, y)), str(x)

    # dict comprehension
    hash_dict = {key: val for key, val in gen_hash_dict(256)}

    # initial message
    message = [int(hash_dict[str(0) + '.' + str(digest[0])])]
    digest = digest[1:]

    # full dehash
    for i, h in enumerate(digest):
        message.append(int(hash_dict[str(message[i]) + '.' + str(digest[i])]))
